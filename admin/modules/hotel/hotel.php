...
class hotel extends vnit{
    ...
    function ds(){
        ...
        $data['listhotel_chanel'] = $this->hotel->get_all_hotel_chanel();
        $config['base_url'] = base_url().'hotel/'.$this->uri->segment(2).'/';
        $config['suffix'] = '.html';
        $config['total_rows']   =  $this->hotel->get_num_hotel($key_word, $catid);
        $data['num'] = $config['total_rows'];
        $config['per_page']  =   20; 
        $config['uri_segment'] = 3; 
        $this->load->library('pagination');
        $this->pagination->initialize($config);   
        $data['list'] =   $this->hotel->get_all_hotel($config['per_page'],segment(3,'int'),$field, $order,$key_word, $catid);
    ...
    function add(){
        ...
        if($this->form_validation->run() === FALSE){
            ...
            $catinfo = $this->hotel->get_cat_by_id($data['catid']);
            if($catinfo->parent_id == 0){
                ...
                $catinfo1 = $this->hotel->get_cat_by_id($catinfo->parent_id);
            ...
            if($_FILES["userfile"]["size"] > 0){
                $config['upload_path'] = ROOT.'data/hotel/default/';
                ...
                if ( !$this->upload->do_upload()){
                    ...
                    vnit_resize_image(ROOT.'data/hotel/default/'.$data['images'],ROOT.'data/hotel/80/'.$data['images'],80,0,false);
                    vnit_resize_image(ROOT.'data/hotel/default/'.$data['images'],ROOT.'data/hotel/200/'.$data['images'],200,0,false);
                    vnit_resize_image(ROOT.'data/hotel/default/'.$data['images'],ROOT.'data/hotel/300/'.$data['images'],300,0,false);
                }                    
            }
    ...
    function edit(){
        ...
        $data['save'] = true;
        $data['apply'] = true;
        $data['cancel'] = 'hotel/ds/'.$this->uri->segment(4);
        $id = segment(3,'int');
        $data['listhotel_chanel'] = $this->hotel->get_all_hotel_chanel();
        $data['rs'] = $this->hotel->get_hotel_by_id($id);
        $row = $this->db->row("SELECT * FROM hotel_chanel WHERE cat_id=".$data['rs']->catid);
        if($row->parent_id == 0){
            $cat = $row->cat_id;
        }else{
            $cat = $row->parent_id;
        }
        $data['channel'] = $this->hotel->get_list_channel($cat);        
        // Form validation
        $this->form_validation->set_rules('data[title]','Tiêu đề','required');
        $this->form_validation->set_rules('data[catid]','','');
        $this->form_validation->set_rules('fulltext','','');
        ...
        if($this->form_validation->run() === FALSE){
            ...
            if($this->db->update('hotel',$data,array('id'=>$id))){
                $this->session->set_flashdata('message','Lưu thành công');
                $option =  $_POST['option'];
                if($option == 'save'){
                    $url = 'hotel/ds/'.$this->uri->segment(4);
                }else{
                    $url = uri_string();
                }
                redirect($url);
    ...
    function del(){
        ...
        $rs = $this->hotel->get_hotel_by_id($id);
        if($rs->images != ''){
            if(file_exists(ROOT.'data/hotel/80/'.$rs->images)){
                unlink(ROOT.'data/hotel/80/'.$rs->images);
            }
            if(file_exists(ROOT.'data/hotel/200/'.$rs->images)){
                unlink(ROOT.'data/hotel/200/'.$rs->images);
            }
            if(file_exists(ROOT.'data/hotel/300/'.$rs->images)){
                unlink(ROOT.'data/hotel/300/'.$rs->images);
            }
            if(file_exists(ROOT.'data/hotel/default/'.$rs->images)){
                unlink(ROOT.'data/hotel/default/'.$rs->images);
            }
        }
    ...
    function get_channel(){
        ...
        $row = $this->db->row("SELECT * FROM hotel_chanel WHERE cat_id=".$catid);
        ...
        if($catid != 0){
            ...
            $list = $this->hotel->get_list_channel($cat);
            foreach($list as $rs):
    ...
    function write_cache(){
        ...
        $listcat = $this->hotel->get_list_maincat();
        $total_cat = count($listcat);
        $str = "<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');\n/**\n* Date: ".date('d/m/y H:i:s').".\n**/";
        $str .= "\n\$config['total_cat'] = $total_cat;";
        $i = 1;
        foreach($listcat as $val):
            $list = $this->hotel->get_list_top($val->cat_id);
            $catid = $val->cat_id;
        ...
        $i++;
        endforeach;
        write_file(ROOT.'site/config/config_hotel.php', $str);    
    }
    
    function cache_tinmoi(){
        $list = $this->hotel->get_tinmoi();
        $this->load->helper('file');
        $str = "<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');\n/**\n* Date: ".date('d/m/y H:i:s').".\n**/";
        $i = 1;
        foreach($list as $rs):
        ...
        $i++;
        endforeach;
        write_file(ROOT.'site/config/config_tophotel.php', $str); 
    }
    
    function cache_noibat(){
        $list = $this->hotel->get_noibat(6);
        $this->load->helper('file');
        $str = "<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');\n/**\n* Date: ".date('d/m/y H:i:s').".\n**/";
        $i = 1;
        foreach($list as $rs):
        ...
        $i++;
        endforeach;
        write_file(ROOT.'site/config/config_hotel_noibat.php', $str); 
    }    
    
    function cache_menu(){
        $list = $this->hotel->get_noibat();
        ...
        $this->load->helper('file');
        $str = "<div class=\"hotel_HightLight\">";
            $str .= "<div class=\"BlockTop\">";
                $str .= "<a class=\"img\" href=\"".$linkTop."\">";
                    $str .='<img alt="'.$titleTop.'" src="'.base_url_site().'data/hotel/300/'.$imgTop.'" style="display: inline;">';
                ...
                for($i = 1; $i < count($list); $i++){
                    ...
                    $str .='<li>';
                        $str .='<a class="imgR" href="'.$link.'">';
                            $str .='<img alt="'.$rs->title.'" src="'.base_url_site().'data/hotel/80/'.$img.'" style="display: inline;">';
                        $str .='</a>';
            ...
            $str .= '</div>';
        $str .= "</div>";
        $str .= '<div class="hotel_Most">';
            $str .= '<h3>Tin xem nhiều nhất</h3>';
            $str .= '<ul>';
                $docnhieu = $this->hotel->get_docnhieu();
        ...
        write_file(ROOT.'site/config/hotel_top.html', $str);
    }
}
