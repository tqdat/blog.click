<?php echo form_open('tour/category/dels',  array('id' => 'admindata'));?> 
...
<table class="admindata">
    ...
    <tr class="row<?php echo $k?>">
        ...
        <td><a href="<? echo site_url('tour/category/edit/'.$rs->cat_id)?>"><?php echo $rs->name?></a></td>
        <td align="center">
        ...
        <td align="center">
            <?php echo icon_edit('tour/category/edit/'.$rs->cat_id)?>
            <span id="publish<?php echo $rs->cat_id?>"><?php echo icon_active("'tour_cat'","'cat_id'",$rs->cat_id,$rs->published)?></span>
        </td>
    </tr>
        <?
        ...
        <tr class="row<?php echo $k?>">
            ...
            <td>|___<a href="<? echo site_url('tour/category/edit/'.$rs1->cat_id)?>"><?php echo $rs1->name?></a></td>
            <td align="center">
            ...
            <td align="center">
                <? echo icon_edit('tour/category/edit/'.$rs1->cat_id)?>
                <span id="publish<? echo $rs1->cat_id?>"><? echo icon_active("'tour_cat'","'cat_id'",$rs1->cat_id,$rs1->published)?></span>
            </td>
        </tr>
            <?
            ...
            <tr class="row<?=$k?>">
                ...
                <td>|___|___<a href="<?=site_url('tour/category/edit/'.$rs2->cat_id)?>"><?php echo $rs2->name?></a></td>
                <td align="center">
                    <input type="text" class="order" name="order_<?=$rs2->cat_id?>" value="<?=$rs2->cat_order?>">
                    <input type="hidden" name="id[]" value="<?=$rs2->cat_id?>">
                </td>
                <td align="center">
                    <?php echo icon_edit('tour/category/edit/'.$rs2->cat_id)?>
                    <span id="publish<?=$rs2->cat_id?>"><?php echo icon_active("'tour_cat'","'cat_id'",$rs2->cat_id,$rs2->published)?></span>
                </td>
            </tr>
            <?endforeach;?>
        <?endforeach;?>
...
function save_order(){
    ...
    $.post(base_url+"tour/category/save_order",fields, function(data) {
        //load_hide();
        location.reload();
    });
}
