...
<table class="admindata">
    ...
    <tr class="row<?php echo $k?>">
        ...
        <td><a href="<? echo site_url('adv/home/edit/'.$rs->id)?>"><?php echo $rs->name?></a></td>
        <td>
            <?if($rs->vitri == 1){echo "Vị trí 1";}else{ echo "Vị trí 2";}?>
        </td>
        <td align="center"><?=$rs->clicker?></td>
        <td><?=date('d/m/Y H:i',$rs->date_begin)?></td>
        <td><?=date('d/m/Y H:i',$rs->date_end)?></td>
        <td align="center">
            <?php echo icon_edit('adv/home/edit/'.$rs->id)?>
            <span id="publish<?php echo $rs->id?>"><?php echo icon_active("'adv_home'","'id'",$rs->id,$rs->published)?></span>
            <?php echo icon_del('adv/home/del/'.$rs->id)?>
        </td>
    </tr>
    <?
    $k = 1 - $k;
