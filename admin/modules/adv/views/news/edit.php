...
<table class="form">
    ...
    <tr>
        ...
        <td>
            <select name="vdata[vitri]">
                <?foreach($vitri as $val):?>
                <option value="<?=$val->vitri?>" <?=($val->vitri == $rs->vitri)?'selected="selected"':''?>><?=$val->tenvitri?> (<?=$val->width?> x <?=$val->height?>)</option>
                <?endforeach;?>
            </select>
        </td>
    </tr>
    <tr>
        ...
        <td>
            <p><input type="checkbox" name="sa" id="sa" onclick="check_chose('sa', 'ar_id[]', 'admindata')"><b>Chọn tất cả</b></p>
            <ul>
                <?foreach($listcat as $val):
                $item = $this->adv->check_item_cat_news($val->cat_id, $rs->id);
                $check = ($item)?'checked="checked"':'';
                ?>
                <li style="float: left;width: 200px;">
                <input type="checkbox" name="ar_id[]" <?=$check?> value="<?=$val->cat_id?>">
                <?=$val->cat_name?>
                </li>
                <?endforeach;?>
            </ul>
