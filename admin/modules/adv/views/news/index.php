...
<table class="admindata">
    ...
    <tr class="row<?php echo $k?>">
        ...
        <td><a href="<? echo site_url('adv/news/edit/'.$rs->id)?>"><?php echo $rs->name?></a></td>
        <td>
            <?=$this->db->row("SELECT tenvitri FROM adv_news_vitri WHERE vitri = ".$rs->vitri)->tenvitri?>
        </td>
        <td align="center"><?=$rs->clicker?></td>
        <td><?=date('d/m/Y H:i',$rs->date_begin)?></td>
        <td><?=date('d/m/Y H:i',$rs->date_end)?></td>
        <td align="center">
            <?php echo icon_edit('adv/news/edit/'.$rs->id)?>
            <span id="publish<?php echo $rs->id?>"><?php echo icon_active("'adv_news'","'id'",$rs->id,$rs->published)?></span>
            <?php echo icon_del('adv/news/del/'.$rs->id)?>
        </td>
    </tr>
    <?
    $k = 1 - $k;
