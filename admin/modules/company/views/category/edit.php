...
<table class="form">
    <tr>
        ...
        <select name="vdata[parent_id]" style="width: 408px;">
            <?foreach($list as $row):
            ?>
                <option value="<?=$row->catid?>" <?=($row->catid == $rs->parent_id)?'selected="selected"':''?>><?=$row->catname?></option>
            <?endforeach;?>
        </select>
        </td>
    </tr>
    <tr>
        ...
        <td><input type="text" name="vdata[catname]" value="<?php echo $rs->catname?>" style="width: 400px;"></td>
    </tr>
    <tr>
