...
function loadSoap($URL_SERVICE = "") 
{
    ini_set("soap.wsdl_cache_enabled", "0"); // disabling WSDL cache
    try {
        $clientObj = new SoapClient($URL_SERVICE, array( 
                                                        'connection_timeout'=>WEBSERVICE_TIMEOUT, 
                                                        'encoding'=>'UTF-8', 
                                                        'style' => SOAP_RPC, 
                                                        'use' => SOAP_ENCODED 
                                                 ) 
...
function get_week_of_day($day){
    $date = strtotime($day); 
    $weekNumber = date("W", $date)-1; 
    return $weekNumber;
}
...
//       $Y = %Y returned from strftime
    ...
    for($k = $week_Start[$W]; $k <= $week_End[$W]; $k = $k + 86400){
        $result[] = $k;
    }
    //$result = array("start" => $week_Start[$W], "end" => $week_End[$W]);
...
function daysWeek($week_nr, $year, $startday=1) {
    ...
    for($i=0; $i<7; $i++){
        ...
        $re[] = strtotime($year.'W'.$setweek.$setday);
    }
...
function doc_tag($data, $tag){
    if (preg_match_all("=<".$tag."[^>]*>(.*)</".$tag.">=siU", $data, $out)) {
        return  $out[1][0];
    }else{
        return "";
    }
...
//Chuyen dung lương File
function vnit_fileSizeInfo($size) 
{ 
